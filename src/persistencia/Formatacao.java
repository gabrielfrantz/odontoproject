...
public class Formatacao {
    ...
    public static String txfDouble(String dado) {
        String retorno = "";
        for (int i = 0; i < dado.length(); i++) {
            if (dado.charAt(i) != 'R' && dado.charAt(i) != '$' && dado.charAt(i) != ' ' && dado.charAt(i) != '/' && dado.charAt(i) != '-'
    ...
    public static String ajustaHoraHM(String hora) {
        String horaFormatada = null;
        try {
            Date date = new Date();
            Date horaHM = new SimpleDateFormat("HH:mm:ss").parse(hora);
            horaFormatada = new SimpleDateFormat("HH:mm").format(horaHM);
        } catch (Exception e) {
            System.err.println(e);
        }
